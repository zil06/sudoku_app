...
## 7. 大量データ投入における起動失敗
### 現象
data.sql に問題文を追加して起動したところ、追加した問題が表示されなかったり、問題ページが立ち上がらず「テーブルが見つからない」というエラーが発生した。

### 原因
追加した問題がデータベースにINSERTされていなかった。
また、データベースファイルを削除して初期化した後も、問題文がデータベースにINSERTされない状態となっていた。

### 解決策
application.properties の設定を変更し、SQLスクリプトからCREATE TABLE・INSERTを実行するよう修正した。

```Properties
# Hibernateによる自動生成をオフにし、SQLスクリプトを優先
